...
const AssignmentForm = ({ defaultValues, courses, onSubmit, isSubmitting, submitLabel = "Save" }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    <form onSubmit={handleSubmit(submit)} className="space-y-4" noValidate>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          />
          {errors.deadline && <p className={errorClasses}>{errors.deadline.message}</p>}
        </div>
      ...
      <div>
        <label className={labelClasses}>Attachment URL (optional)</label>
        <input className={inputClasses} placeholder="https://..." {...register("attachment")} />
      </div>
